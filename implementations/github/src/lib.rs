...
impl ModuleTrait for Module {
    fn try_resolve_uri(args: ArgsTryResolveUri, _env: Option<Env>) -> Result<Option<UriResolverMaybeUriOrManifest>, String> {
        ...
        let response = match result {
            Ok(Some(x)) => x,
            _ => return Ok(Some(UriResolverMaybeUriOrManifest {
                uri: None,
                manifest: None
            }))
        };
        ...
        if let Some(body) = response.body {
            let content = decode(body).map_err(|e|
                format!("GitHub URI Resolver Extension: failed to decode URI.txt from base64: {}", e.to_string())
            )?;
            let uri = from_utf8(content.as_slice()).map_err(|e|
                format!("GitHub URI Resolver Extension: failed to parse URI.txt from bytes: {}", e.to_string())
            )?.to_string();
